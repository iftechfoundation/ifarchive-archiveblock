...
The `<If>` directive ensures that the plugin applies only to existing files (and symlinks to files), not directories or missing files. This prevents the plugin from interfering with Apache's `mod_autoindex` directory generator. I don't know why it interferes, but it does.
